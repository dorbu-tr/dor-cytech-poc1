...
<!-- Source: .cursor/skills/core/cluster-deployment/SKILL.md | Regenerate: .specify/scripts/sh/sync-ai-rules.sh claude-code -->
---
name: cluster-deployment
description: >-
  Determines the correct production AKS cluster for deploying the service based
  on domain ownership, database access, and service type. Use when the user asks
  "which cluster", "where to deploy", "deployment target", "production cluster",
  "cluster recommendation", or any similar intent to determine the correct
...
- `.claude/skills/team/` contains `trading-*` skill names → Trading
