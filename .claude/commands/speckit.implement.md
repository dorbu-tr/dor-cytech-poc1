...
<!-- Source: .cursor/commands/speckit.implement.md | Regenerate: .specify/scripts/sh/sync-ai-rules.sh claude-code -->
...
6. **Multi-Agent Delegation** (for eToro .NET API services):
   ...
   **How to delegate:**
   ...
   - When delegating, **ALWAYS** start the prompt with:
     1. "FIRST: Read your core agent file at `.claude/agents/core/<name>.md` — it lists which skills to load"
     2. "AND: If `.claude/agents/team/<name>.md` exists, read it too — it extends the core agent with team-specific skills"
